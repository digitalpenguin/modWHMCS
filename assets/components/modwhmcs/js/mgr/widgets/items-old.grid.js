...
Ext.extend(modWHMCS.grid.Items,MODx.grid.Grid,{
    ...
    ,getMenu: function() {
        var m = [];
        m.push({
            text: _('modwhmcs.item_update')
            ,handler: this.updateItem
        });
        m.push('-');
        m.push({
            text: _('modwhmcs.item_remove')
    ...
    ,createUpdateItem: function(btn,e,isUpdate) {
        ...
        this.windows.createUpdateItem = MODx.load({
            ...
            ,isUpdate: isUpdate
            ,title: (isUpdate) ?  _('modwhmcs.item_update') : _('modwhmcs.item_create')
            ,record: r
            ,listeners: {
    ...
    ,removeItem: function(btn,e) {
        ...
        MODx.msg.confirm({
            title: _('modwhmcs.item_remove')
            ,text: _('modwhmcs.item_remove_confirm')
            ,url: this.config.url
            ,params: {
...
modWHMCS.window.CreateUpdateItem = function(config) {
    config = config || {};
    this.ident = config.ident || 'modwhmcs-mecitem'+Ext.id();
    Ext.applyIf(config,{
        id: this.ident
        ,closeAction: 'close'
        ,url: modWHMCS.config.connectorUrl
